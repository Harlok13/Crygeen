...
class MenuEventHandler:
    ...
    def __click_button(self, buttons_list: list[Button]) -> None:
        for button in buttons_list:
            if button.rect.collidepoint(pg.mouse.get_pos()):
                ...
                self.main_menu.status = button.input(self.main_menu, self.game)

    def screensaver_event_handler(self, event: Event) -> None:
        if event.type == pg.KEYDOWN:
            self.main_menu.status = Status.MAIN_MENU
            self.main_menu.menu.dropdown_start_time = pg.time.get_ticks()

    def main_menu_event_handler(self, event: Event) -> None:
        if event.type == pg.MOUSEBUTTONDOWN and event.button == 1:
            self.__click_button(self.main_menu.menu.menu_buttons_list)

    def exit_menu_event_handler(self, event: Event) -> None:
        if event.type == pg.MOUSEBUTTONDOWN and event.button == 1:
            self.__click_button(self.main_menu.exit_menu.exit_buttons_list)

        elif event.type == pg.KEYDOWN:
            if event.key == pg.K_ESCAPE:
                self.main_menu.status = Status.MAIN_MENU
                self.main_menu.exit_menu.exit_dropdown_start_time = pg.time.get_ticks()
    ...
    def settings_menu_event_handler(self, event: Event) -> None:
        if event.type == pg.MOUSEBUTTONDOWN:
            if event.button == 1:
                self.__click_button(self.main_menu.settings_menu.control_buttons_list)
    ...
    def control_menu_event_handler(self, event: Event) -> None:
        if event.type == pg.KEYDOWN:
            if event.key == pg.K_ESCAPE:
                self.main_menu.status = Status.SETTINGS
            else:
